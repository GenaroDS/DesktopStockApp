    ...
    void modifyButtonOnAction(ActionEvent event) {
        ...
        try {
            ...
            if (!itemNameField.getText().equals(itemName)) {
                ...
                query = "UPDATE items SET item_name = '" + itemNameField.getText() + "' WHERE item_id='" + itemId
                        + "';";
                statement.executeUpdate(query);

            }
            if (!itemPriceField.getText().equals(itemPrice)) {
                query = "UPDATE items SET item_amount = '" + itemPriceField.getText() + "' WHERE item_id='" + itemId
                        + "';";
                statement.executeUpdate(query);
            }
            if (!itemQuantityField.getText().equals(itemQuantity)) {
                query = "UPDATE items SET item_price = '" + itemQuantityField.getText() + "' WHERE item_id='" + itemId
                        + "';";
                statement.executeUpdate(query);
            }
            App.setRoot("ItemsScene");
