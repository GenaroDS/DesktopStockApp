...
public class ItemsSceneController implements Initializable {
    ...
    void addItemOnAction(ActionEvent event) throws IOException {
        ...
        if (itemName.length() > 0 && itemQty.length() > 0 && itemPrice.length() > 0) {
            String query = "INSERT INTO items (user_id,item_name,item_price,item_amount) VALUES('" + userId + "','"
                    + itemName + "','" + itemPrice + "','"
                    +  itemQty + "');";
            try {
    ...
    void deleteItemOnAction(ActionEvent event) {
        ModelTable selectedItem = itemsTable.getSelectionModel().getSelectedItem();
        ...
        try {
            java.sql.Statement statement = connection.createStatement();
            String query = "DELETE FROM items WHERE user_id='" + userId + "' AND item_name='" + itemName + "';";
            statement.executeUpdate(query);
            connection.close();
            initialize(null, null);
        } catch (SQLException e) {
    ...
    public void refreshTable(){
        ...
        try {
            ...
            while (rs.next()) {
                oblist.add(new ModelTable(rs.getString("item_id"), rs.getString("item_name"), rs.getString("item_amount"),
                        rs.getString("item_price"), rs.getString("user_id")));
            }
